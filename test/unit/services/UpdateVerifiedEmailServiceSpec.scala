...
class UpdateVerifiedEmailServiceSpec extends PlaySpec with MockitoSugar with BeforeAndAfterEach with ScalaFutures {
  ...
  UpdateVerifiedEmailResponse(ResponseCommon("OK", None, "processingDate", List(MessagingServiceParam(formBundleIdParamName, "testValue")))))
  private val businessErrorUpdateVerifiedEmailResponse = VerifiedEmailResponse(
    UpdateVerifiedEmailResponse(ResponseCommon("OK", Some("004 - Duplicate Acknowledgement Reference"), "processingDate", List(MessagingServiceParam(positionParamName, Fail)))))
