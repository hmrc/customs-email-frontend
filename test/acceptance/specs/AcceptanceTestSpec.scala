...
trait AcceptanceTestSpec extends FeatureSpec
  ...
  private def encryptSessionData(sessionData: Map[String, String]): String = {
    ...
    lazy val sessionCookieCrypto: CryptoGCMWithKeysFromConfig = new ApplicationCrypto(config).SessionCookieCrypto
    val UTF_8 = "UTF-8"
    lazy val provider: Option[String] = app.configuration.getOptional[String]("application.crypto.provider")
    lazy val applicationSecret = app.configuration.getOptional[String]("play.crypto.secret").getOrElse("some random secret").getBytes
  ...
  def addUserInSession() = {
    webDriver manage() addCookie new Cookie("mdtp", encryptSessionData(Map(SessionKeys.userId -> "/auth/oid/some-oid")))
  }
